...
function complexPasswordCheck(pwd, {name, phone, birth} = {}) {
	// 密码长度不小于8位，由大小写字母、数字以及特殊符号组成。	
	var reg = /^([a-zA-Z0-9]|[! @ # $ % ^ & * + - _ =]){8,20}$/;
	if (!reg.test(pwd)) {
		return '密码以字母开头，长度不小于8位，由大小写字母、数字以及特殊符号!@#$%^&*+-_=组成'
	}
	...
	if (pwd.match && pwd.toLocaleLowerCase().match(repetReg)) {
		return "不要出现3位及以上重复的字母、数字、特殊字符，如AAA、Aaa、111、###等"
	}
  const hander = checkKeyBoard()
  if (pwd.split("").some(item => !hander(String(item).toLocaleLowerCase()))) {
	  return "不要出现3位及以上连续的字母、数字、特殊字符，如ABC、Abc、123、！@#等"
	}
	return ""
}
